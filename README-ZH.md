...
## 🚀 DealSim 2.0：100% 本地化且高性价比（无需 Zep Cloud）

与原本严重依赖 **Zep Cloud**（收费的外部 GraphRAG 和实体提取云服务，每月费用高达 $350+）的旧版本不同，**DealSim 2.0 开箱即用，完全实现本地化**：
- **零云端内存依赖**：所有知识图谱本体生成、交易主张解析、实体提取和时间关系映射均通过自定义的本地客户端架构 (`LocalZep`) 在本地处理。
- **巨大的成本节省**：完全免去了昂贵的 Zep Cloud 订阅费用和海量的外部 API 调用账单。
- **极佳的隐私性与极速响应**：敏感的投资备忘录、财务报表和路演 PPT 绝不会离开您的机器，且能以极低的离线延迟流畅运行。
...
---
© 2026 Nursan Omarov. DealSim 2.0 - 机构级交易压力测试引擎.
