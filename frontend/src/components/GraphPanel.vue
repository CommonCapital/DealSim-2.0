<template>
  <div class="graph-panel">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          <div class="memory-icon-wrapper">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="memory-icon pulse-red" v-if="isSimulating">
              <path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" />
            </svg>
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="memory-icon" v-else>
              <path d="M9.5 2A2.5 2.5 0 0 1 12 4.5v15a2.5 2.5 0 0 1-4.96.44 2.5 2.5 0 0 1-2.96-3.08 3 3 0 0 1-.34-5.58 2.5 2.5 0 0 1 1.32-4.24 2.5 2.5 0 0 1 4.44-4.04z" />
            </svg>
          </div>
          {{ isSimulating ? 'IC Room: Adversarial Audit in Progress...' : 'Mapping Investment Claims...' }}
        </div>
        ...
        <div v-if="selectedItem" class="detail-panel">
          ...
          <div v-if="selectedItem.type === 'node'" class="detail-content">
            ...
            <!-- DealSim Scrutiny Score Panel -->
            <div class="detail-section scrutiny-panel" v-if="selectedItem.data.scrutiny_score !== undefined">
              <div class="section-title">Fragility Assessment:</div>
              <div class="scrutiny-score-container">
                <div class="score-meta">
                  <span class="score-label">Scrutiny Failure Rate</span>
                  <span class="score-val">{{ (selectedItem.data.scrutiny_score * 100).toFixed(0) }}%</span>
                </div>
                <div class="score-track">
                  <div class="score-bar" :style="{ width: (selectedItem.data.scrutiny_score * 100) + '%', background: getScrutinyColor(selectedItem.data.scrutiny_score) }"></div>
                </div>
                <div class="score-verdict" :style="{ color: getScrutinyColor(selectedItem.data.scrutiny_score) }">
                  {{ selectedItem.data.scrutiny_score > 0.7 ? 'CRITICAL RISK' : selectedItem.data.scrutiny_score > 0.4 ? 'SIGNIFICANT RESISTANCE' : 'MODERATE SCRUTINY' }}
                </div>
              </div>
            </div>
    ...
    <div v-if="graphData && entityTypes.length" class="graph-legend">
      ...
      <div class="legend-section fragility-legend">
        <span class="legend-title">Scrutiny Level</span>
        <div class="legend-items">
          <div class="legend-item"><span class="legend-dot status-safe"></span><span class="legend-label">Solid</span></div>
          <div class="legend-item"><span class="legend-dot status-warn"></span><span class="legend-label">Weakened</span></div>
          <div class="legend-item"><span class="legend-dot status-danger"></span><span class="legend-label">Critical</span></div>
        </div>
      </div>
    </div>
...
const renderGraph = () => {
  ...
  const nodeGroup = g.append('g').attr('class', 'nodes')
    ...
    // Scrutiny Ring (Fragility Visualization)
    const scrutinyRing = nodeGroup.selectAll('circle.scrutiny-ring')
      .data(nodes.filter(d => d.rawData.scrutiny_score !== undefined))
      .enter().append('circle')
      .attr('class', 'scrutiny-ring')
      .attr('r', 16)
      .attr('fill', 'none')
      .attr('stroke', d => getScrutinyColor(d.rawData.scrutiny_score))
      .attr('stroke-width', 3)
      .attr('stroke-dasharray', d => d.rawData.scrutiny_score > 0.4 ? '4,2' : 'none')
      .style('opacity', d => 0.3 + (d.rawData.scrutiny_score * 0.7))
      .style('pointer-events', 'none')

    const node = nodeGroup.selectAll('circle.main-node')
      .data(nodes)
      .enter().append('circle')
