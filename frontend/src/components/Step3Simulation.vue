  ...
  <div class="simulation-panel">
    ...
    <div class="control-bar">
      <div class="status-group">
        <!-- IC Room 状态 (DealSim 专属) -->
        <div class="platform-status ic-room" :class="{ active: runStatus.runner_status === 'running', completed: runStatus.runner_status === 'completed' }">
          <div class="platform-header">
            <svg class="platform-icon" viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M3 21h18"></path>
              <path d="M3 7v1h18V7a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2z"></path>
          ...
          <div class="platform-stats">
            ...
            <span class="stat">
              ...
              <span class="stat-value mono">{{ allActions.length || 0 }}</span>
            </span>
          </div>
          <!-- 可用动作提示 -->
          <div class="actions-tooltip">
            <div class="tooltip-title">Active Mandates</div>
            <div class="tooltip-actions">
              <span class="tooltip-action">ATTACK CLAIMS</span>
              <span class="tooltip-action">EVIDENCE AUDIT</span>
              <span class="tooltip-action">LOGIC STRESS-TEST</span>
              <span class="tooltip-action">RISK SYNTHESIS</span>
              <span class="tooltip-action">DILIGENCE GAP ANALYSIS</span>
            </div>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div class="timeline-card">
              <div class="card-header">
                ...
                <div class="header-meta">
                  <div class="platform-indicator">
                    <svg v-if="action.platform === 'ic_room' || action.platform === 'parallel'" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2">
              ...
              <div class="card-body">
                ...
                <!-- INTERROGATION: 质询 -->
                <div v-if="action.action_type === 'INTERROGATION' && action.result" class="content-text main-text interrogation-text">
                  <div class="stage-tag" v-if="action.action_args?.stage">{{ action.action_args.stage }}</div>
                  {{ action.result }}
                </div>

                <!-- 通用回退：未知类型或有 content 但未被上述处理 -->
                <div v-if="!['CREATE_POST', 'QUOTE_POST', 'REPOST', 'LIKE_POST', 'CREATE_COMMENT', 'SEARCH_POSTS', 'FOLLOW', 'UPVOTE_POST', 'DOWNVOTE_POST', 'DO_NOTHING', 'INTERROGATION'].includes(action.action_type) && action.action_args?.content" class="content-text">
                  {{ action.action_args.content }}
                </div>
              </div>
...
// 审计开始以来的模拟时长
const auditElapsedTime = computed(() => {
...
const getActionTypeClass = (type) => {
  const classes = {
    ...
    'INTERROGATION': 'badge-interrogate',
...
.platform-status.ic-room .platform-icon { color: #E63946; }
