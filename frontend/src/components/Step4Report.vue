        ...
        <div v-if="reportOutline" class="report-content-wrapper">
          ...
          <div class="report-header-block">
            <div class="report-meta">
              <span class="report-tag">IC PREP REPORT</span>
              <span class="report-id">DEAL-SIM-{{ reportId?.slice(0, 8).toUpperCase() || 'REF-X92' }}</span>
            </div>
            ...
            >
              ...
              <div class="section-body" v-show="!collapsedSections.has(idx)">
                ...
                <div v-if="generatedSections[idx + 1]" class="generated-content">
                  <!-- Special Card for Investment Recommendation -->
                  <div v-if="isRecommendationSection(section.title)" class="verdict-card">
                    <div class="verdict-status" :class="getVerdictClass(generatedSections[idx + 1])">
                      <div class="verdict-label">Investment Verdict</div>
                      <div class="verdict-value">{{ getVerdictText(generatedSections[idx + 1]) }}</div>
                    </div>
                    <div class="verdict-score">
                      <div class="score-label">Confidence Score</div>
                      <div class="score-value">{{ getConfidenceScore(generatedSections[idx + 1]) }}%</div>
                      <div class="score-bar">
                        <div class="score-fill" :style="{ width: getConfidenceScore(generatedSections[idx + 1]) + '%' }"></div>
                      </div>
                    </div>
                  </div>
                  
                  <div v-html="renderMarkdown(generatedSections[idx + 1])"></div>
                </div>
...
// DealSim Verdict Logic
const isRecommendationSection = (title) => {
  const t = title.toLowerCase()
  return t.includes('recommendation') || t.includes('投资建议')
}

const getVerdictText = (content) => {
  if (content.includes('GO-GO') || content.includes('GO')) return 'GO'
  if (content.includes('NO-GO')) return 'NO-GO'
  return 'DEFERRED'
}

const getVerdictClass = (content) => {
  const text = getVerdictText(content)
  if (text === 'GO') return 'status-go'
  if (text === 'NO-GO') return 'status-nogo'
  return 'status-deferred'
}

const getConfidenceScore = (content) => {
...
const renderMarkdown = (content) => {
  ...
  html = html.replace(/^### (.+)$/gm, '<h4 class="md-h4">$1</h5>')
  ...
  // 处理表格 (Markdown Table Support)
  // 先处理表头和对齐分隔符，最后处理行
  html = html.replace(/^\|(.+)\|(\s*\n)\|?\s*[:\- ]+[:\- | ]+[:\- ]+\s*\|?(\s*\n)((^\|.+|(\s*\n)?)*)/gm, (match, headerRow, nl1, nl2, bodyRows) => {
    const headers = headerRow.split('|').filter(h => h.trim() !== '').map(h => `<th class="md-th">${h.trim()}</th>`).join('')
...
/* Status variants */
.status-go .verdict-value { color: #059669; }
.status-go { color: #059669; }
.verdict-card.status-go::before { background: #059669; }

.status-nogo .verdict-value { color: #DC2626; }
.status-nogo { color: #DC2626; }
.verdict-card.status-nogo::before { background: #DC2626; }

.status-deferred .verdict-value { color: #D97706; }
.status-deferred { color: #D97706; }
.verdict-card.status-deferred::before { background: #D97706; }
