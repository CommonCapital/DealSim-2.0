...
from ..utils.local_zep import get_zep_client

from ..config import Config
from ..utils.logger import get_logger
from ..utils.locale import get_locale, set_locale

logger = get_logger('dealsim.zep_graph_memory_updater')
...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        ...
        action_descriptions = {
            ...
            "INTERROGATION": self._describe_interrogation,
        }
    ...
    def _describe_interrogation(self) -> str:
        """投审会质询 - 包含阶段名和具体内容"""
        stage = self.action_args.get("stage", "审计阶段")
        content = self.action_args.get("content", "") or self.action_args.get("result", "")
        
        if content:
            return f"在投审会「{stage}」阶段进行了深度质询：「{content}」"
        return f"在投审会「{stage}」阶段发表了见解"
