...
logger = get_logger('dealsim.simulation_runner')
...
class SimulationRunner:
    ...
    def start_simulation(
        cls,
        simulation_id: str,
        platform: str = "ic_room",  # twitter / reddit / parallel / ic_room
        max_rounds: int = None,
        enable_graph_memory_update: bool = False,
        graph_id: str = None
    ) -> SimulationRunState:
        """
        启动模拟 (DealSim IC Room 优先支持)
        """
        ...
        script_map = {
            ...
            "ic_room": "run_ic_room_simulation.py",
            "parallel": "run_parallel_simulation.py"
        }
        script_name = script_map.get(platform, "run_ic_room_simulation.py")
        ...
        try:
            ...
            logger.info(f"模拟启动成功 [{platform}]: {simulation_id}, pid={process.pid}")
    ...
    def _monitor_simulation(cls, simulation_id: str, locale: str = 'zh'):
        ...
        ic_log = os.path.join(sim_dir, "ic_actions.jsonl")
        ...
        positions = {"twitter": 0, "reddit": 0, "ic_room": 0}
        ...
        try:
            ...
            logger.error(f"监控线程异常: {simulation_id}, {e}")
            state.runner_status = RunnerStatus.FAILED
            state.error = str(e)
            cls._save_run_state(state)
        finally:
    ...
    def _read_action_log(cls, log_path: str, position: int, state: SimulationRunState, platform: str) -> int:
        ...
        try:
            with open(log_path, 'r', encoding='utf-8') as f:
                ...
                for line in f:
                    ...
                    try:
                        ...
                        # 更新阶段信息 (IC Room 专用)
                        if platform == "ic_room" and "stage" in action.action_args:
                            state.current_stage = action.action_args["stage"]
            ...
            logger.warning(f"读取日志失败 {log_path}: {e}")
            return position
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        ic_enabled = os.path.exists(ic_log)
