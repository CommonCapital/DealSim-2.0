...
class Config:
    ...
    # Flask配置
    SECRET_KEY = os.environ.get('SECRET_KEY', 'dealsim-secret-key')
    DEBUG = os.environ.get('FLASK_DEBUG', 'True').lower() == 'true'
    
    # PROJECT NAME
    PROJECT_NAME = "DealSim"
    ...
    OASIS_DEFAULT_MAX_ROUNDS = int(os.environ.get('OASIS_DEFAULT_MAX_ROUNDS', '5')) # IC Room stages are distinct
    OASIS_SIMULATION_DATA_DIR = os.path.join(os.path.dirname(__file__), '../uploads/simulations')
    
    # DealSim IC Room 动作 (Attacking/Supporting claims)
    IC_ROOM_ACTIONS = [
        'CHALLENGE_CLAIM', 'SUPPORT_CLAIM', 'REQUEST_DILIGENCE', 'SUBMIT_VERDICT', 'DO_NOTHING'
    ]
